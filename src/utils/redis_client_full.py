"""
Redis client utility for the content recommendation engine.
...
class RedisConnectionManager:
    ...
    async def get_async_client(self) -> aioredis.Redis:
        ...
        if not self._async_client or not await self._is_async_connection_healthy():
    ...
    async def _create_async_client(self):
        ...
        try:
            ...
            self.logger.info(f"Connected to Redis (async) at {self.config.host}:{self.config.port}")
    ...
    async def _is_async_connection_healthy(self) -> bool:
        """Check if async connection is healthy"""
        try:
            current_time = time.time()
            if current_time - self._last_health_check > self.config.health_check_interval:
            ...
            self.logger.warning(f"Redis async health check failed: {e}")
            self._is_healthy = False
            return False
    ...
    async def close_async(self):
        ...
        try:
            ...
            self.logger.info("Closed Redis async connections")
        except Exception as e:
            self.logger.error(f"Error closing Redis async connections: {e}")
...
class RedisClient:
    ...
    async def async_client(self) -> aioredis.Redis:
        """Get the underlying async Redis client"""
        return await self.connection_manager.get_async_client()
    
    def is_healthy(self) -> bool:
        """Check if Redis is healthy"""
        return self.connection_manager._is_connection_healthy()
    ...
    async def close_async(self):
        """Close async connections"""
        await self.connection_manager.close_async()
